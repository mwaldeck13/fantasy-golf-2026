 ...
 *   - Auth             (Name | PIN)
 *   - Schedule         (Week | Event | Deadline (datetime cell))
...
function getSchedule(ss) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (!wk) continue;
    const event = String(rows[i][1] || '').trim();
    const ddl = rows[i][2] ? (rows[i][2] instanceof Date ? rows[i][2] : new Date(rows[i][2])) : null;
...
function fetchEspnScoreboard(yyyymmdd) {
  const url = ESPN_SCOREBOARD + (yyyymmdd ? '?dates=' + yyyymmdd : '');
  ...
  if (!json.events || !json.events.length) {
    throw new Error('No event in ESPN scoreboard for ' + (yyyymmdd || 'current'));
  }
...
function syncField(week) {
  ...
  const yyyymmdd = Utilities.formatDate(sched.deadline, TZ, 'yyyyMMdd');

  const lb = fetchEspnScoreboard(yyyymmdd);
  if (lb.eventName && sched.event && !nameMatch_(lb.eventName, sched.event)) {
    appendLog_('syncField', week, 'WARN', `ESPN event '${lb.eventName}' doesn't match Schedule '${sched.event}' — proceeding anyway`);
  }
