...
// ESPN now 403s Apps Script. UrlFetchApp always sends its own
// "Mozilla/5.0 (compatible; Google-Apps-Script;…)" user agent — it IGNORES any
// User-Agent you set — and ESPN blocks that string, so ESPN can't be called directly
// from Apps Script. Route every ESPN request through a tiny Cloudflare Worker proxy
// that refetches the URL with an allowed user agent. Paste your Worker URL below
// (ending in "/?url="). Leave '' to call ESPN directly (only works off a non-blocked
// network — e.g. if ESPN lifts the block later).
const ESPN_PROXY = 'https://espn-proxy.mwaldeck13.workers.dev/?url=';

// All ESPN requests go through this so they use the proxy when one is configured.
function espnFetch_(url) {
  const target = ESPN_PROXY ? ESPN_PROXY + encodeURIComponent(url) : url;
  return UrlFetchApp.fetch(target, { muteHttpExceptions: true });
}
...
function installTriggers() {
  ...
  // Field sync Mon/Tue/Wed 8am ET. ESPN often doesn't post the field until mid-week, so
  // retry each morning: syncField no-ops when the field is empty and just refreshes it
  // once ESPN posts it — all three days land before the Thursday 7am deadline.
  for (const day of [ScriptApp.WeekDay.MONDAY, ScriptApp.WeekDay.TUESDAY, ScriptApp.WeekDay.WEDNESDAY]) {
    ScriptApp.newTrigger('syncFieldNextWeek').timeBased()
      .onWeekDay(day).atHour(8).inTimezone(TZ).create();
  }
  ...
  SpreadsheetApp.getUi().alert('Installed: field sync Mon/Tue/Wed 8am + results sync Sun 9pm (Eastern).');
}
...
function fetchEspnEarnings(eventId) {
  const res = espnFetch_(ESPN_CORE_EVENT + eventId);
  if (res.getResponseCode() !== 200) {
...
function computeMadeCut_(eventId) {
  ...
  for (const c of comps) {
    ...
    if (!ref) continue;
    try {
      const st = JSON.parse(espnFetch_(ref).getContentText());
...
function syncField(week) {
  ...
  // ESPN often lists the event days before it posts the field. Bail loudly instead of
  // wiping the week's existing rows with an empty field (which read as a false "OK").
  if (!lb.athletes.length) {
    throw new Error(`ESPN has no field posted yet for Week ${week}: ${lb.eventName}. Try again closer to the tournament.`);
  }
