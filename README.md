...
- **Leaderboard** — total earnings, rank, winner count, credits remaining, and **average finishing place**. Tap any row to expand a per-week breakdown.
...
- **Rosters** — per participant: full pick history with each golfer tagged Reusable / Locked / Winner, plus a summary line of credits earned, spent, and average place.
...
Each participant's average finishing position across their scored picks, shown on the Leaderboard and Rosters (the Leaderboard still ranks by earnings — average place is just an extra stat). Finishes convert to numbers: `1`→1, `T35`→35. A missed cut (`MC`/`WD`/`DQ`) counts as **(players who made the cut) + 1** — so it needs the cut size for that week (see [Cut column](#cut-column-average-place)). Any missed-cut pick whose cut size isn't available yet is left out of the average and marked with a `*`.
...
- **Average place** = mean of finishing positions across scored picks; `MC`/`WD`/`DQ` = (made-cut count) + 1.

This matches the rules in `fantasy-golf-league-summary.md`.

## The Chase tab

An end-of-season projection view, driven by projected payouts you enter on the Schedule tab for the closing tournaments (see [Projected payouts](#projected-payouts-the-chase-tab)). Because scores accumulate independently (participants may pick the same golfer, and nobody's gain costs anyone else), the math is exact:

- **Ceiling** = a player's current total plus the winner's payout of every remaining event (their best case: win out).
- **Best / worst possible finish** = computed from every player's ceiling vs. floor.
- **The chase** = for each trailing player, the gap to first and the minimum number of remaining wins (biggest purses first) needed to erase it.
